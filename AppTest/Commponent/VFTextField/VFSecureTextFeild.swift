...
public struct VFSecureTextFeild: View {
    ...
    /// Creates a new secure text field with floating label, state management, and built-in password visibility toggle.
    ///
    /// - Parameters:
    ///   - text: Binding to the text value that will be displayed and edited in the secure text field.
    ...
    ///   - iconIdentifier: Optional accessibility identifier for the password visibility toggle icon for testing purposes.
    ///   - textFieldIdentifier: Optional accessibility identifier for the text field for testing purposes.
    ///   - iconSize: Optional custom size for the password visibility toggle icon. Defaults to the standard icon size from constants.
    ///   - onChange: Optional closure called whenever the text value changes.
    ///
    /// - Note: The secure text field automatically handles state transitions:
    ...
    ///   - Includes built-in password visibility toggle (eye/eye.slash icon)
    ...
    private var secureTextFeild: some View {
        HStack{
            SecureTextField(text: $text, isSecure: isSecure) { editing in
                if state != .disabled {
                    if editing {
                        state = .editing
                    } else if state != .error {
            ...
            .frame(height: 26) // Fixed height to match text field
            ...
            iconView(icon: Image(systemName: isSecure ? "eye.slash" : "eye"), action: {isSecure.toggle()})
        }
    }
...
#Preview {
    struct VFSecureTextFeildPreview: View {
        ...
        var body: some View {
            VStack{
                /// How to use
                VFSecureTextFeild(text: $text,
                                  state: $state,
                                  floatingText: "Employee Num: 010xxxxx",
                                  normalText: "Enter your text",
    ...
    return VFSecureTextFeildPreview()
}
